...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div style="margin:50px;">
		<c:if test="${!empty orderInfo }">
			...
			<div>开卡日期：${orderInfo.order_time }</div>
			<div>开卡场站：${orderInfo.airport_name }</div>
			<div>业务员：${orderInfo.name }</div>
			<div>业务员电话：${orderInfo.phone }</div>
		</c:if>
		<hr>
		...
		<c:if test="${!empty postInfo }">
			<div>发票信息：${postInfo.invoice_flag }</div>
			<div>邮寄地址：${postInfo.address }</div>
			<div>发票类型：${postInfo.invoice_type }</div>
			<div>发票抬头：${postInfo.invoice_title }</div>
		</c:if>
		<hr>
