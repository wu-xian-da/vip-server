	...
	public Map<String,Object> pageList(@RequestParam(value="page",defaultValue="1") Integer pageNo,
		...
		for(AppUserFeedback appUserFeedback : feedbackList){
			if(appUserFeedback.getFeedbackState() == 0){//反馈信息未处理
				...
				appUserFeedback.setOpr("<a href='gotoFeedBackDetailView?userId="+appUserFeedback.getUserId()+"&feedBackId="+appUserFeedback.getId()+"'><button class='btn'>查看</button></a><button class='btn btn-back' onclick='delFeedBackInfo("+appUserFeedback.getId()+")'>删除</button>");
			}
		}
