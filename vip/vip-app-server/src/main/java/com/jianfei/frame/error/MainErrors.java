...
public class MainErrors {
    ...
    public static MainError getError(MainErrorType mainErrorType, Locale locale, Object... params) {
        ...
        if (params.length > 0) {
            errorSolution = String.format(errorSolution, params);
        }
        return new SimpleMainError(mainErrorType.value(), errorMessage, errorSolution);
    }

    public static MainError getErrorExt(MainErrorType mainErrorType, Object data, Locale locale, Object... params) {
        String errorMessage = getErrorMessage(ERROR_CODE_PREFIX + mainErrorType.value(), locale);
        String errorSolution = getErrorSolution(ERROR_CODE_PREFIX + mainErrorType.value() + ERROR_SOLUTION_SUBFIX, locale);
        ...
        if (data == null) return new SimpleMainError(mainErrorType.value(), errorMessage, errorSolution);
        else return new SimpleMainErrorExt(mainErrorType.value(), errorMessage, errorSolution, data);
    }
