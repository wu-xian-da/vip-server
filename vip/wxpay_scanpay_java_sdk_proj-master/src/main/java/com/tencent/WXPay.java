...
public class WXPay {
    ...
    /**
     * 扫码支付请求支付服务（业务逻辑已封装完毕，推荐调用）
     * @param nativePayReqData 这个数据对象里面包含了API要求提交的各种数据字段
     * @return API返回的数据
     * @throws Exception
     */
    public static NativePayResData doNativePayBusiness(NativePayReqData nativePayReqData, NativePayBusiness.ResultListener resultListener) throws Exception {
        return new NativePayBusiness().run(nativePayReqData, resultListener);
    }
    
    /**
     * 扫码支付请求支付服务（需要调用方处理xml格式转换，回调信息处理）
     * @param nativePayReqData 这个数据对象里面包含了API要求提交的各种数据字段
     * @return API返回的数据
     * @throws Exception
     */
    public static String requestNativePayService(NativePayReqData nativePayReqData) throws Exception{
        return new NativePayService().request(nativePayReqData);
    }
    
    /**
     * 扫码支付请求查询服务
     * @param nativePayQueryReqData 这个数据对象里面包含了API要求提交的各种数据字段
     * @return API返回的数据
     * @throws Exception
     */
